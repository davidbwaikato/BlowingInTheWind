...
window.bitws_registerSocketOn = function(socket) {
	...
	console.log("**** !!!! Not changing speed.  Need to think about this some more!!");
	//viewer.clock.multiplier = (data_speed) ? data_speed : 1.0;
    });

    socket.on("auto_change_speed", (data_speed) => {
	console.log("socket.on('auto_change_speed'): speed =", data_speed)
	bitw_speed_auto_changed = true;
...
window.isPaused = function() {
    ...
    if (viewer!= null && viewer.clock.shouldAnimate) {
	isPaused = false;
    }
...
if (viewer != null) {
	...
	.subscribe(function(shouldAnimate) {
		...
		if (bitw_animation_auto_started) {
		    // consume the 'auto-started'
		    bitw_animation_auto_started = false;
		}
		else {
		    // 'bitw_animation_auto_started' state means this was generated by manual/user click
		...
		/*
		// refactor into function that returns a message rec
		const date_now = new Date();
		const hours = date_now.getHours();
		const mins  = date_now.getMinutes();
		
		const time_str = hours + ":" + String(mins).padStart(2,"0");
		const key = "message-paused-"+MessagePausedNum;
		MessagePausedNum++;
		
		const message_data = {
		    author: "BITW Assistant",
		    message: "Suspending hints while paused",
		    time: time_str,
		    key: key
		};
		// Send message to self (!!) via socket (to reach BitwChat.js)
		bitw_socket.emit("receive_message",message_data);
		*/
    ...
    Cesium.knockout.getObservable(viewer.clockViewModel, 'multiplier')
	.subscribe(function(newMultiplier) {
	    if (newMultiplier < 0) {
		console.log("newMultiplier is negative (going back in time!), so ignore for other BITW player");
	    }
	    if (newMultiplier == 0) {
		console.log("newMultiplier is 0 (stationary!), so ignore for other BITW player");
	    }
	    else if (newMultiplier < 1) {
		console.log("newMultiplier is < 1 (too slow!), so ignore for other BITW player");
	    }
	    else if (newMultiplier > 30) {
		console.log("newMultiplier is > 30 (too fast!), so ignore for other BITW player");
	    }
	    else {

		// If 'bitw_spped_auto_changed' is false, then this means the multiplier change was caused by a user-click
		// => Tell the server to initiate 'start_speedchange' for all players in this room

		if (bitw_speed_auto_changed) {
		    // consume the 'auto-started'
		    bitw_speed_auto_changed = false;
		}
		else {
		    // 'bitw_animation_auto_started' state means this was generated by manual/user click
		    console.log("**** bitw_socket.emit('start_speedchange_in_room')", bitw_roomId, newMultiplier);
		    bitw_socket.emit('start_speedchange_in_room', bitw_roomId, newMultiplier);
		}
	    }	    
	});
